...
mkdir -p $LOGS_FOLDER
echo "$R Script started executing at: $(date) $N" &>>$LOG_FILE
if [ $USERID -ne 0 ]
then
...
if [ $? -ne 0 ]
then
    echo -e "$R MySQL is not installed...initialising installation $N" &>>$LOG_FILE
...
if [ $? -ne 0 ]
then
    echo -e "$R python3 is not installed...initialising installation $N" &>>$LOG_FILE
...
if [ $? -ne 0 ]
then
    echo -e "$R Nginx is not installed...initialising installation $N" &>>$LOG_FILE
